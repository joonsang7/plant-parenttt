...
public class App {
    public static void main(String[] args) {
        ...
        /**
         * EDT에서 GUI를 생성하는 이유! :
         * 1. 스레드 안전성: Swing 컴포넌트는 스레드 안전하지 않기 때문에, GUI 관련 작업은 반드시 EDT에서 실행해야 하는데 invokeLater()를 사용하면 GUI 작업이 EDT에서 실행되도록 보장할 수 있습니다.
         * 2. 응답성 유지: GUI 작업이 메인 스레드에서 실행되면, 긴 작업이 GUI를 멈추게 할 수 있습니다. EDT에서 GUI를 생성하면, 메인 스레드는 다른 작업을 계속 수행할 수 있어 응답성을 유지할 수 있습니다.
         * 3. 일관된 GUI 상태: EDT에서 GUI를 생성하면, 모든 GUI 관련 작업이 동일한 스레드에서 실행되므로, GUI 상태가 일관되게 유지됩니다. 이는 예기치 않은 동시성 문제를 방지하는 데 도움이 됩니다.
         */
